...
def component_diagnostics(
    ...
    for index, sample_id in enumerate(sample_ids):
        ...
        psf_support_energy = float((np.abs(target_output) * psf_support).sum())
        total_target_output = float(np.abs(target_output).sum())
        proxy_energy = float(proxy.sum())
        source_energy = float(maps["S"].sum())
        residual_energy = float(np.abs(maps["R"]).sum())
        residual_target_energy = float((np.abs(maps["R"]) * psf_support).sum())
        ...
        psf_only_error = float((np.abs(maps["T_psf"] - proxy) * psf_support).sum())
        combined_target_error = float((np.abs(target_output - proxy) * psf_support).sum())
        residual_detail_gain = (
            (psf_only_error - combined_target_error) / proxy_energy
            if proxy_energy > EPS
            else None
        )
        ...
        record: dict[str, object] = {
            ...
            "psf_residual_overlap": _safe_ratio(
                float((np.abs(maps["T_psf"]) * np.abs(maps["R"])).sum()),
            ...
            "residual_meaningful": bool(
                has_target
                and residual_detail_gain is not None
