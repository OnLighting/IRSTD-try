...
def _moment_matched_psf_teacher(
    target_proxy: Tensor,
    mask: Tensor,
    psf_support: Tensor,
    sigma_min: float = 0.6,
    ...
    """Project each labelled target onto an energy-matched Gaussian base."""
    height, width = target_proxy.shape[-2:]
    y_grid, x_grid = torch.meshgrid(
        torch.arange(height, device=target_proxy.device, dtype=torch.float32),
        torch.arange(width, device=target_proxy.device, dtype=torch.float32),
        indexing="ij",
    )
    teacher = torch.zeros_like(target_proxy, dtype=torch.float32)
    masks = mask.detach().to(device="cpu", dtype=torch.uint8).numpy()
    structure = np.ones((3, 3), dtype=np.uint8)
    ...
    for batch_index in range(mask.shape[0]):
        ...
        assigned = labelled[tuple(nearest)]
        assigned_tensor = torch.as_tensor(assigned, device=target_proxy.device)
        proxy = target_proxy[batch_index, 0].float()
        support = psf_support[batch_index, 0].float()

        for component in range(1, count + 1):
            weights = proxy * (assigned_tensor == component)
            energy = weights.sum()
            if float(energy.detach().cpu()) <= 1e-8:
                continue
            mean_y = (weights * y_grid).sum() / energy
            mean_x = (weights * x_grid).sum() / energy
            dy = y_grid - mean_y
            dx = x_grid - mean_x
            covariance = torch.stack(
                (
                    torch.stack(((weights * dx.square()).sum(), (weights * dx * dy).sum())),
                    torch.stack(((weights * dx * dy).sum(), (weights * dy.square()).sum())),
                )
            ) / energy
            eigenvalues, eigenvectors = torch.linalg.eigh(covariance)
            eigenvalues = eigenvalues.clamp(sigma_min**2, sigma_max**2)
            inverse = eigenvectors @ torch.diag(eigenvalues.reciprocal()) @ eigenvectors.T
            exponent = -0.5 * (
                inverse[0, 0] * dx.square()
                + 2.0 * inverse[0, 1] * dx * dy
                + inverse[1, 1] * dy.square()
            )
            gaussian = torch.exp(exponent) * support
            gaussian = gaussian / gaussian.sum().clamp_min(1e-12)
            teacher[batch_index, 0] += energy * gaussian

    return teacher.to(dtype=target_proxy.dtype)
