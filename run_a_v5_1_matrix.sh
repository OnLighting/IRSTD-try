...
validate_checkpoint() {
  local checkpoint="$1"
  local dataset="$2"
  "$PYTHON_BIN" - "$checkpoint" "$dataset" "$SEED" <<'PY'
import sys
import torch

path, expected_dataset, expected_seed = sys.argv[1], sys.argv[2], int(sys.argv[3])
...
run_train() {
  ...
  if [[ -f "$marker" && -f "$best" ]]; then
    validate_checkpoint "$best" "$dataset"
  ...
  if [[ -f "$last" ]]; then
    validate_checkpoint "$last" "$dataset"
    command+=(--resume "$last")
...
metrics_are_complete() {
  local metrics="$1"
  local dataset="$2"
  "$PYTHON_BIN" - "$metrics" "$dataset" <<'PY'
import json
import sys
from pathlib import Path

path, source = Path(sys.argv[1]), sys.argv[2]
if not path.is_file():
    raise SystemExit(1)
...
run_eval() {
  ...
  if [[ "$FORCE_EVAL" == "0" ]] && metrics_are_complete "$metrics" "$dataset"; then
  ...
  metrics_are_complete "$metrics" "$dataset"
}
