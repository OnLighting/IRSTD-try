...
# One paid-GPU invocation for Stage-A v5.2a signed-R repair: verify, train/resume,
# evaluate, visualize.
# Run from the new_model_v3 repository root.

RUN_DIR="${RUN_DIR:-runs/a_psf/irstd1k_seed42_v5_2a_signed_r}"
...
if [[ -f "$RUN_DIR/a_last.pt" ]]; then
  echo "[A-v5.2a] resuming from $RUN_DIR/a_last.pt"
  ...
  echo "[A-v5.2a] starting a new seed-$SEED run in $RUN_DIR"
fi
