...
def _perfect_case() -> tuple[torch.Tensor, torch.Tensor, dict, dict]:
    ...
    targets = {
        ...
        "psf_teacher": psf.clone(),
        "residual_teacher": torch.zeros_like(psf),
...
def test_signed_residual_diagnostics_use_absolute_energy_and_measure_gain() -> None:
    """Catches positive and negative R energy cancelling in diagnostics."""
    image, mask, prediction, targets = _perfect_case()
    prediction["T_psf"][..., 4, 4] = 0.8
    prediction["R"][..., 4, 4] = -0.2
    targets["psf_teacher"][..., 4, 4] = 0.8
    targets["residual_teacher"][..., 4, 4] = -0.2

    record = component_diagnostics(image, mask, prediction, targets, ["signed"])[0]

    assert record["residual_abs_energy_ratio"] == pytest.approx(1 / 3)
    assert record["residual_teacher_nmae"] == pytest.approx(0.0)
    assert record["residual_detail_gain"] == pytest.approx(1 / 3)
    assert record["residual_outside_abs_fraction"] == pytest.approx(0.0)
...
def test_degeneration_flags_detect_known_collapses() -> None:
    summary = {
        ...
        "residual_abs_energy_ratio_median": 0.97,
        "residual_teacher_nmae_median": 0.85,
        "target_contrast_recall_median": 8.0,
        "target_psf_zero_fraction": 0.80,
        "background_input_correlation_median": 0.9999,
        "residual_meaningful_fraction": 0.0,
    }
    gates = {
        "source_false_activation_max": 0.20,
        "residual_abs_energy_ratio_max": 0.90,
        "residual_teacher_nmae_max": 0.50,
