...
def test_losses_prefer_the_identifiable_signed_teacher_decomposition() -> None:
    """Catches PSF consuming all target energy while the signed R branch dies."""
    image = torch.full((1, 1, 32, 32), 0.2)
    mask = torch.zeros_like(image)
    image[..., 15:18, 15:18] = 0.55
    image[..., 16, 16] = 0.95
    image[..., 15, 17] = 0.80
    mask[..., 15:18, 15:18] = 1
    targets = build_weak_targets(image, mask, dilation_radius=2, ring_radius=5)
    ...
    matched = _controlled_prediction(image)
    matched["T_psf_raw"] = targets["psf_teacher"].clone()
    matched["R"] = targets["residual_teacher"].clone()
    shortcut = copy.deepcopy(matched)
    shortcut["T_psf_raw"] = targets["target_proxy"].clone()
    shortcut["R"].zero_()

    loss = APSFUnmixingLoss(_weights())
    _, matched_terms = loss(image, mask, matched, targets)
    _, shortcut_terms = loss(image, mask, shortcut, targets)

    assert matched_terms["target"] < shortcut_terms["target"]
    assert matched_terms["residual"] < shortcut_terms["residual"]


def test_signed_detail_overlap_is_not_treated_as_psf_residual_collapse() -> None:
    """Catches the old overlap penalty rejecting a valid local correction."""
    image, mask = _synthetic_batch()
    targets = build_weak_targets(image, mask, dilation_radius=2, ring_radius=5)
    prediction = _controlled_prediction(image)
    prediction["T_psf_raw"][..., 15, 15] = 0.5
    prediction["R"][..., 15, 15] = -0.2

    _, terms = APSFUnmixingLoss(_weights())(image, mask, prediction, targets)

    assert terms["independence"].item() < 0.1


def test_residual_loss_prefers_the_signed_teacher_over_the_zero_branch() -> None:
    """Catches an R objective whose optimum remains the all-zero branch."""
    image = torch.full((1, 1, 32, 32), 0.2)
    mask = torch.zeros_like(image)
    image[..., 16, 16] = 0.9
    mask[..., 16, 16] = 1
    targets = build_weak_targets(image, mask, dilation_radius=2, ring_radius=5)
    zero = _controlled_prediction(image)
    matched = copy.deepcopy(zero)
    matched["R"] = targets["residual_teacher"].clone()

    loss = APSFUnmixingLoss(_weights())
    _, zero_terms = loss(image, mask, zero, targets)
    _, matched_terms = loss(image, mask, matched, targets)

    assert matched_terms["residual"] < zero_terms["residual"]
...
def test_residual_is_free_to_explain_supported_non_psf_target_energy() -> None:
    ...
    inside["R"] = targets["residual_teacher"].clone()
...
def test_teacher_psf_tail_inside_kernel_footprint_is_not_leakage() -> None:
    """Catches measuring a teacher PSF with the narrower source support."""
    image = torch.full((1, 1, 32, 32), 0.2)
    mask = torch.zeros_like(image)
    image[..., 16, 16] = 0.9
    mask[..., 16, 16] = 1
    ...
    assert matched_terms["target"] < 1e-5
...
def test_independence_does_not_penalize_psf_residual_overlap() -> None:
    image, mask = _synthetic_batch()
    targets = build_weak_targets(image, mask, dilation_radius=2, ring_radius=5)
    prediction = _controlled_prediction(image)
