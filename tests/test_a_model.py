...
def test_a_forward_returns_bounded_full_resolution_components() -> None:
    ...
    assert torch.allclose(out["U"], out["u_rec"], atol=1e-6)
    assert out["reconstruction_raw"].shape == image.shape


def test_signed_residual_starts_at_zero_and_can_correct_psf_overshoot() -> None:
    """Catches a sigmoid residual that cannot emit a negative correction."""
    model = build_a_model(dims=(8, 16, 32), num_psf=3, kernel_size=7)
    image = torch.rand(1, 1, 32, 32)

    initial = model(image)["R"]
    torch.testing.assert_close(initial, torch.zeros_like(initial))

    with torch.no_grad():
        model.residual_head.bias.fill_(-1.0)
    corrected = model(image)["R"]
    assert torch.all(corrected < 0)


def test_uncertainty_supervision_isolated_from_shared_features() -> None:
    """Catches U-loss gradients degrading the shared decomposition backbone."""
    model = build_a_model(dims=(8, 16, 32), num_psf=3, kernel_size=7)

    uncertainty = model(torch.rand(1, 1, 32, 32))["U"]
    uncertainty.mean().backward()

    assert any(parameter.grad is not None for parameter in model.uncertainty_head.parameters())
    assert all(
