...
def test_psf_teacher_conserves_target_energy_and_leaves_signed_detail() -> None:
    """Catches a model-dependent or non-signed residual supervision target."""
    image = torch.full((1, 1, 25, 25), 0.2)
    mask = torch.zeros_like(image)
    image[..., 11:14, 11:14] = 0.55
    image[..., 12, 12] = 0.95
    image[..., 11, 13] = 0.80
    mask[..., 11:14, 11:14] = 1
    ...
    teacher = out["psf_teacher"]
    detail = out["residual_teacher"]
    assert torch.all(teacher >= 0)
    assert teacher.sum().item() == pytest.approx(
        out["target_proxy"].sum().item(), rel=1e-5
    )
    ...
    assert detail.min().item() < 0
    assert detail.max().item() > 0
