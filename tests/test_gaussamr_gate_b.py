...
class GateBSupportCoverageTest(unittest.TestCase):
    def test_center_hit_outside_mahalanobis_support_is_not_covered(self):
        mask = torch.zeros(1, 32, 32)
        mask[0, 16, 16] = 1
        proposals = torch.tensor(
            [[[0.9, 8.0, 16.0, 1.0, 1.0, 0.0]] * 8], dtype=torch.float32
        )

        diagnostics = detail_support_diagnostics(proposals, mask, k2=8)

        self.assertEqual(diagnostics["targets"], 1)
        self.assertEqual(diagnostics["support_fraction_sum"], 0.0)
        self.assertEqual(diagnostics["fully_supported"], 0)

    def test_support_uses_probability_uncertainty_k2_selection(self):
        mask = torch.zeros(1, 64, 64)
        mask[0, 31:34, 31:34] = 1
        proposals = torch.zeros(1, 9, 6)
        proposals[0, :8, 0] = torch.tensor(
            [0.90, 0.80, 0.79, 0.78, 0.77, 0.76, 0.75, 0.60]
        )
        proposals[0, :8, 1:3] = 4.0
        proposals[0, :8, 3:5] = 1.0
        proposals[0, 8] = torch.tensor([0.59, 32.0, 32.0, 2.0, 2.0, 1.0])

        diagnostics = detail_support_diagnostics(proposals, mask, k2=8)

        self.assertEqual(diagnostics["targets"], 1)
        self.assertEqual(diagnostics["support_fraction_sum"], 1.0)
        self.assertEqual(diagnostics["fully_supported"], 1)
...
class GateBEvaluationTest(unittest.TestCase):
    def test_reports_target_additive_coverage_and_both_mask_metrics(self):
        ...
        self.assertEqual(metrics["support_coverage_at_8"], 1.0)
        self.assertEqual(metrics["fully_supported_at_8"], 2)
