...
def test_v5_config_has_exact_dual_source_loss_weights() -> None:
    ...
    assert config["loss"]["weights"]["uncertainty"] == pytest.approx(0.1)
    assert config["loss"]["objective_version"] == "v5.2a-signed-r"
...
def test_validation_score_penalizes_residual_missing_its_teacher() -> None:
    summary = {
        "reconstruction_mae_mean": 0.02,
        "target_energy_precision_median": 0.9,
        "target_contrast_recall_median": 0.9,
        ...
        "background_target_leakage_median": 0.1,
        "psf_residual_overlap_median": 0.2,
        "residual_teacher_nmae_median": 0.1,
        "residual_detail_gain_mean": 0.05,
    }
    shortcut = dict(summary, residual_teacher_nmae_median=0.9)

    assert validation_score(summary) < validation_score(shortcut)


def test_validation_score_penalizes_a_harmful_signed_residual() -> None:
    """Catches checkpoint selection accepting R that worsens PSF-only fit."""
    summary = {
        "reconstruction_mae_mean": 0.02,
        "target_energy_precision_median": 0.9,
        ...
        "uncertainty_error_spearman_median": 0.6,
    }
    harmful = dict(summary, residual_detail_gain_mean=-0.1)

    assert validation_score(summary) < validation_score(harmful)
...
def test_validation_score_penalizes_uncertainty_below_the_public_gate() -> None:
    summary = {
        "reconstruction_mae_mean": 0.02,
        "target_energy_precision_median": 0.9,
        "target_contrast_recall_median": 1.0,
    ...
    anticorrelated = dict(summary, uncertainty_error_spearman_median=-0.2)

    assert validation_score(summary) < validation_score(anticorrelated)


def test_validation_score_penalizes_a_dead_residual_branch() -> None:
    summary = {
        "reconstruction_mae_mean": 0.02,
        "target_energy_precision_median": 0.9,
        "target_contrast_recall_median": 1.0,
    ...
    dead = dict(summary, residual_meaningful_fraction=0.0)

    assert validation_score(summary) < validation_score(dead)
