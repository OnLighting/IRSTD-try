...
def test_signed_residual_panel_uses_symmetric_zero_centered_limits() -> None:
    """Catches negative PSF corrections being clipped out of the R panel."""
    values = torch.tensor([[-0.3, 0.1]], dtype=torch.float32).numpy()

    assert _display_limits("R", values) == pytest.approx((-0.3, 0.3))
