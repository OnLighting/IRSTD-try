...
def _display_limits(title: str, values: np.ndarray) -> tuple[float, float]:
    """Return display limits, centering signed residual corrections at zero."""
    if title == "R":
        finite = values[np.isfinite(values)]
        magnitude = max(float(np.abs(finite).max()), 1e-6) if finite.size else 1.0
        return -magnitude, magnitude
    return 0.0, _display_vmax(title, values)
...
def save_component_panel(
    ...
    for axis, (title, values, color_map) in zip(axes.flat, panels):
        display_min, display_max = _display_limits(title, values)
        shown = axis.imshow(values, cmap=color_map, vmin=display_min, vmax=display_max)
        axis.set_title(
